...
class graph2graph(object):
    def __init__(self, sess,Ds, No, HNo, Nr, HNr, Dr, De_o, De_r, Mini_batch, checkpoint_dir,epoch,Ds_inter,Dr_inter,Step):
        self.sess = sess
        self.Ds = Ds
        self.No = No
        self.HNo = HNo
    ...
    def build_model(self): # output 是 O_2 和 Ra_2
       ...
       self.O_1 = tf.placeholder(tf.float32, [self.mini_batch_num,self.Ds,self.No], name="node_data_train") # node_data_train, (50,1,200) data图 batch_num, The State Dimention, The Number of Objects
       self.O_target = tf.placeholder(tf.float32, [self.mini_batch_num,self.Ds,self.No], name="node_label_train") # node_label_train, (50,1,200) label图

       # Relation Matrics R=<Rr,Rs,Ra>
       # entity graph 的 源端、目的端、边
       self.Rr = tf.placeholder(tf.float32, [self.mini_batch_num,self.No,self.Nr], name="Rr_data") # Rr_data, (50,200,39800) 每个节点对的源端 batch_num, The Number of Objects, The Number of Relations
       self.Rs = tf.placeholder(tf.float32, [self.mini_batch_num,self.No,self.Nr], name="Rs_data") # Rs_data, (50,200,39800) 每个节点对的目的端
       self.Ra_1 = tf.placeholder(tf.float32, [self.mini_batch_num,self.Dr,self.Nr], name="Ra_data_train") # Ra_data_train, (50,2,39800) data图每个节点对是否有边 batch_num, The Relationship Dimension, The Number of Relations

       # hunk graph 的 源端、目的端、边
       self.Hr = tf.placeholder(tf.float32, [self.mini_batch_num, self.HNo, self.HNr], name="Hr_label")  # Hr_label, (50,74,5476) 每个节点对的源端 batch_num, The Number of Objects, The Number of Relations
       self.Hs = tf.placeholder(tf.float32, [self.mini_batch_num, self.HNo, self.HNr], name="Hs_label")  # Hs_label, (50,74,5476) 每个节点对的目的端
       self.Ra_target = tf.placeholder(tf.float32, [self.mini_batch_num,self.Dr,self.HNr], name="Ra_label_train") # Ra_label_train, (50,2,5476) label图每个节点对是否有边
       # External Effects
       #self.X = tf.placeholder(tf.float32, [self.mini_batch_num,self.Dx,self.No], name="X")

       # entity 边源端、目的端的hunk
       self.RHr = tf.placeholder(tf.float32, [self.mini_batch_num, self.HNo, self.Nr], name="RHr_data") # RHr_data, (50,74,39800)
       self.RHs = tf.placeholder(tf.float32, [self.mini_batch_num, self.HNo, self.Nr], name="RHs_data") # RHs_data, (50,74,39800)
       ...
       self.B_2 = self.m_B2(self.B_1,self.RHr,self.RHs,self.Hr,self.Hs,self.Ra_target) # !!!

       # # updating the node state (node translation)
       ...
       self.H_Ra = self.phi_B2(self.B_2)  # (50,20,5476) add a constrain to make edge1-2 == edge 2-1
       self.H_Ra1 = self.a_R(self.H_Ra,self.Ra_target) # (50,22,5476) aggregation函数 (50,20,5476) 和 (50,2,5476) 在第2个维度拼接
    ...
    def m_B2 (self,B_1,RHr,RHs,Hr,Hs,Ra_target):
        ...
        Hr_t = tf.transpose(Hr,[0,2,1]) # (50,74,5476)->(50,5476,74)
        Hs_t = tf.transpose(Hs,[0,2,1]) # (50,74,5476)->(50,5476,74)
        Hr_neighbors_h = tf.matmul(Hr_t, neighbors) # (50,5476,74)*(50,74,4)=(50,5476,4)
        Hs_neighbors_h = tf.matmul(Hs_t, neighbors) # (50,5476,74)*(50,74,4)=(50,5476,4)
        Hr_neighbors_ht = tf.transpose(Hr_neighbors_h,[0,2,1]) # (50,5476,4)->(50,4,5476)
        Hs_neighbors_ht = tf.transpose(Hs_neighbors_h,[0,2,1]) # (50,5476,4)->(50,4,5476)
        B_2 = tf.concat ([Hr_neighbors_ht,Hs_neighbors_ht,Ra_target],1);#  concat (50,4,5476),(50,4,5476),(50,2,5476)的第1个维度=(50,10,5476)
        return B_2;
    ...
    def a_O(self,E,Rr,O):
       ...
       E_bar = tf.matmul(E,tf.transpose(Rr,[0,2,1])) + tf.matmul(E,tf.transpose(self.Rs,[0,2,1])); #(50,20,200) 把39800抵消了，也即根据边找到对应端点的新的值
    ...
    def phi_B2(self,B2):
     with tf.variable_scope("phi_B2") as scope:
       h_size = 20; # B2: (50,10,5476)
       B_trans = tf.transpose(B2,[0,2,1]); # (50,5476,10)
       # HNr:5476 Number of Relations; Ds:1 State Dimention; Dr:2 Relationship Dimension
       # (5476*50, 10) = (273800,10)
       ...
       w1 = tf.Variable(tf.truncated_normal([B2.shape[1], h_size], stddev=0.1), name="w1", dtype=tf.float32); # (10,20)
       ...
       h2_trans = tf.reshape(h2,[self.mini_batch_num,self.HNr,self.De_r]); # (50,5476,20)
       h2_trans = tf.transpose(h2_trans,[0,2,1]); # (50,20,5476) 每个B的特征

       # edge translation
       # tf.transpose(self.Rr,[0,2,1]):(50,5476,20)
       # tf.transpose(self.Rs,[0,2,1]):(50,5476,20)
       h2_trans_bar1 = tf.matmul(h2_trans,tf.transpose(self.Hr,[0,2,1])); # (50,20,74) B2根据源端hunk排序 (50,20,5476)*(50,5476,74)
       h2_trans_bar2 = tf.matmul(h2_trans,tf.transpose(self.Hs,[0,2,1])); # (50,20,74) B2根据目的端hunk排序 (50,20,5476)*(50,5476,74)
       # (50,20,74)*(50,74,5476) = (50,20,5476) 源端hunk 和 目的hunk 分别还原成关系的矩阵，结合源端hunk 和 目的端hunk的 B2
       effects = tf.matmul(h2_trans_bar1,self.Hr) + tf.matmul(h2_trans_bar2,self.Hs)

       return effects # (50,20,5476)
    ...
    def phi_HRa(self, HRa):
        with tf.variable_scope("phi_U_R1") as scope:
            h_size = 20;
            HRa_trans = tf.transpose(HRa, [0, 2, 1]); # (50,22,5476)->(50,5476,22)
            HRa_trans2 = tf.reshape(HRa_trans, [self.mini_batch_num * self.HNr, HRa.shape[1]]); #(50*5476,22)

            # 要定义成变量，它才是一个变量
            w1 = tf.Variable(tf.truncated_normal([HRa.shape[1], h_size], stddev=0.1), name="HRa_w1", dtype=tf.float32); # (22,20)
            b1 = tf.Variable(tf.zeros([h_size]), name="HRa_b1", dtype=tf.float32);  # (20)
            h1 = tf.nn.relu(tf.matmul(HRa_trans2, w1) + b1);  # (273800,22)*(22,20) + 20 = (273800,20) 每个batch中所有hunk边的维度

            w2 = tf.Variable(tf.truncated_normal([h_size, self.Dr_inter], stddev=0.1), name="o1_w2r", dtype=tf.float32); # (20,2)
            b2 = tf.Variable(tf.zeros([self.Dr_inter]), name="o1_b2r", dtype=tf.float32); # (2)

            h2_trans = tf.reshape(tf.matmul(h1, w2) + b2, [self.mini_batch_num, self.HNr, self.Dr]);  # (50,5476,2)
            h2_trans_logits = tf.transpose(h2_trans, [0, 2, 1]);  # (50,2,5476)
            h2 = tf.nn.softmax(h2_trans_logits, dim=1)  # (50,2,5476)
    ...
    def train(self, args):# 激活(sess.run) model里面的参数
        ...
        for i in range(max_epoches):
          ...
          acc_top = top_ACC(Ra_label_train, np.array(HRa_t).reshape(Ra_label_train.shape[0],Ra_label_train.shape[1],Ra_label_train.shape[2])) # (50,2,5476)
    ...
    def test(self,args):        
        ...
        for j in range(int(len(node_data_test)/self.mini_batch_num)):
          ...
          batch_Ra_target = Ra_label_test[j*self.mini_batch_num:(j+1)*self.mini_batch_num] #sess就像一个指针，处理的地方被激活
        ...
        HRa_t = np.array(HRa_t).reshape(len(node_data_test),self.Dr,self.HNr) # (50,1,5476)
        

        np.save('outputSelf\glide\HRa_t'+str(self.No)+'.npy', np.array(HRa_t).reshape(len(node_data_test),self.Dr,self.HNr))
        np.save('outputSelf\glide\HRa_y'+str(self.No)+'.npy', Ra_label_test.reshape(len(node_data_test),self.Dr,self.HNr))
