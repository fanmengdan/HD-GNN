...
class graph2graph(object):
    ...
    def build_model(self):
       ...
       self.Es = tf.placeholder(tf.float32, [self.mini_batch_num,self.No,self.Nr], name="Es")
       self.Et = tf.placeholder(tf.float32, [self.mini_batch_num,self.No,self.Nr], name="Et")
       ...
       self.B_1=self.m(self.O_1,self.Es,self.Et,self.Ra_1)
       # updating the node state
       self.E_O_1=self.phi_E_O_1(self.B_1)  
       self.C_O_1=self.a_O(self.E_O_1,self.Es,self.O_1)
       ...
       self.B_2=self.m(self.O_2,self.Es,self.Et,self.Ra_2)
       # updating the node state
       self.E_O_2=self.phi_E_O_2(self.B_2)  
       self.C_O_2=self.a_O(self.E_O_2,self.Es,self.O_1)
    ...
    def a_O(self,E,Rr,O):
       E_bar=tf.matmul(E,tf.transpose(Rr,[0,2,1]))+tf.matmul(E,tf.transpose(self.Et,[0,2,1]));
    ...
    def phi_E_R_1(self,B):
     with tf.variable_scope("phi_E_R1") as scope:          
       ...
       h2_trans=tf.transpose(h2_trans,[0,2,1]);
       h2_trans_bar=tf.matmul(h2_trans,tf.transpose(self.Es,[0,2,1]));
       
       effects=tf.matmul(h2_trans_bar,self.Es)+tf.matmul(h2_trans_bar,self.Et)
       return effects  
    ...
    def map_loss(self,k):
         with tf.variable_scope("map_conv") as scope: 
           ...
           loss1=self.map_conv(theta2,self.Ra_2,self.O_2,k)+self.map_conv(theta3,self.Ra_3,self.O_3,k)+self.map_conv(theta1,self.Ra_1,self.O_1,k)
           loss2=tf.sqrt(2*tf.nn.l2_loss(tf.reshape(theta2,[k])))+tf.sqrt(2*tf.nn.l2_loss(tf.reshape(theta3,[k])))+tf.sqrt(tf.nn.l2_loss(tf.reshape(theta1,[k]))*2)
           return loss1/3+0.01*loss2,theta2    
       
       
    def train(self, args):
        optimizer = tf.train.AdamOptimizer(0.0001);
        trainer = optimizer.minimize(50*self.loss_node_mse+self.loss_edge_mse+0.001*self.loss_map+self.loss_para+self.loss_E_O+self.loss_E_R); #

        init_op = tf.global_variables_initializer()
        self.sess.run(init_op)

        #read data
        node_data_train,node_data_test,node_label_train,node_label_test,Ra_data_train,Ra_data_test,Ra_label_train,Ra_label_test,Es_data,Et_data = read_data(self)
        ...
        for i in range(max_epoches):
          ...
          for j in range(int(len(node_data_train)/self.mini_batch_num)):
            ...
            O_t_batch,Ra_t_batch,tr_loss_part_node,tr_loss_part_edge,tr_loss_part_map,theta,_=self.sess.run([self.O_3,self.Ra_3,self.loss_node_mse,self.loss_edge_mse,self.loss_map,self.theta,trainer],feed_dict={self.O_1:batch_O,self.O_target:batch_O_target,self.Ra_1:batch_Ra,self.Ra_target:batch_Ra_target,self.Es:Es_data[:self.mini_batch_num],self.Et:Et_data[:self.mini_batch_num]});
    ...
    def test(self,args):        
        ...
        for j in range(int(len(node_data_test)/self.mini_batch_num)):
          ...
          te_loss_part_node,te_loss_part_edge,te_loss_part_map,Ra_part,O_part=self.sess.run([self.loss_node_mse,self.loss_edge_mse,self.loss_map,self.Ra_3,self.O_3],feed_dict={self.O_1:batch_O,self.O_target:batch_O_target,self.Ra_1:batch_Ra,self.Ra_target:batch_Ra_target,self.Es:Es_data[:self.mini_batch_num],self.Et:Et_data[:self.mini_batch_num]});
