# -*- coding: utf-8 -*-
# 无 entity embedding layer、 syntatic relation embedding layer (HD-GNN/ES)
...
class graph2graph(object):
    ...
    def build_model(self): # output 是 E_node_train2 和 E_edge_train2
       ...
       self.E_node_train = tf.placeholder(tf.float32, [self.mini_batch_num,self.Ds,self.Ne], name="E_node_train") # E_node_train, (50,1,200) data图 batch_num, The State Dimention, The Number of Objects
       # self.O_target = tf.placeholder(tf.float32, [self.mini_batch_num,self.Ds,self.Ne], name="node_label_train") # node_label_train, (50,1,200) label图

       # Relation Matrics R=<Es,Et,E_edge>
       # entity graph 的 源端、目的端、边
       self.Es = tf.placeholder(tf.float32, [self.mini_batch_num,self.Ne,self.Ner], name="Es_data") # Es_data, (50,200,39800) 每个节点对的源端 batch_num, The Number of Objects, The Number of Relations
       self.Et = tf.placeholder(tf.float32, [self.mini_batch_num,self.Ne,self.Ner], name="Et_data") # Et_data, (50,200,39800) 每个节点对的目的端
       self.E_edge_train = tf.placeholder(tf.float32, [self.mini_batch_num,self.Dr,self.Ner], name="E_edge_train") # E_edge_train, (50,2,39800) data图每个节点对是否有边 batch_num, The Relationship Dimension, The Number of Relations

       # hunk graph 的 源端、目的端、边
       self.Cs = tf.placeholder(tf.float32, [self.mini_batch_num, self.Nc, self.Ncr], name="Cs_label")  # Cs_label, (50,74,5402) 每个节点对的源端 batch_num, The Number of Objects, The Number of Relations
       self.Ct = tf.placeholder(tf.float32, [self.mini_batch_num, self.Nc, self.Ncr], name="Ct_label")  # Ct_label, (50,74,5402) 每个节点对的目的端
       self.C_edge_train = tf.placeholder(tf.float32, [self.mini_batch_num,self.Dr,self.Ncr], name="C_edge_train") # C_edge_train, (50,2,5402) label图每个节点对是否有边
       # External Effects
       ...
       self.B_2 = self.marshalling_B2(self.B_1,self.Esc,self.Etc,self.Cs,self.Ct,self.C_edge_train) # !!!

       # # updating the node state (node translation)
       # self.e_E_node_train = self.mlp_entity_B1(self.B_1) # mlp函数 (50,20,39800)
       # self.a_E_node_train = self.agg_entity_B1(self.e_E_node_train,self.Es,self.E_node_train) # aggregation函数 (50,21,200)
       # self.E_node_train2 = self.mlp2_entity_B1(self.a_E_node_train)  # 降维, data图 (50,1,200)
       #
       # #updating the edge (edge translation) !!!!!!主要改 mlp_edge_B1
       # self.e_E_edge = self.mlp_edge_B1(self.B_1)  # (50,20,39800) add a constrain to make edge1-2 == edge 2-1
       # self.a_E_edge = self.agg_edge_B1(self.e_E_edge,self.E_edge_train) # aggregation函数 (50,20,39800) + (50,2,39800)
       # self.E_edge_train2, self.Ra_logits_2 = self.mlp2_edge_B1(self.a_E_edge) # 降维, score被softmax后(50,2,39800), 输出的score(50,2,39800)

       #updating the hunk edge (edge translation) self.mlp_hunk_B2 和 self.mlp_hunkedge_B2 最重要
       self.C_edge_output = self.mlp_hunk_B2(self.B_2)  # (50,20,5402) add a constrain to make edge1-2 == edge 2-1
       self.a_C_edge_output = self.agg_edge_B1(self.C_edge_output,self.C_edge_train) # (50,22,5402) aggregation函数 (50,20,5402) 和 (50,2,5402) 在第2个维度拼接
       self.C_edge_output2, self.C_edge_output2_logits = self.mlp_hunkedge_B2(self.a_C_edge_output) # 降维, score被softmax后(50,2,39800), 输出的score(50,2,39800)
    ...
    def marshalling_B2 (self,B_1,Esc,Etc,Cs,Ct,C_edge_train):
        ...
        B_2 = tf.concat ([Cs_neighboEt_ht,Ct_neighboEt_ht,C_edge_train],1);#  concat (50,4,5402),(50,4,5402),(50,2,5402)的第1个维度=(50,10,5402)
        return B_2;
    ...
    def agg_entity_B1(self,E,Es,O):
       ...
       E_bar = tf.matmul(E,tf.transpose(Es,[0,2,1])) + tf.matmul(E,tf.transpose(self.Et,[0,2,1])); #(50,20,200) 把39800抵消了，也即根据边找到对应端点的新的值
    ...
    def mlp2_entity_B1(self,C):
     with tf.variable_scope("phi_U_O1") as scope:
       ...
       # 要定义成变量，它才是一个变量
       w1 = tf.Variable(tf.truncated_normal([(self.Ds+self.De_e), h_size], stddev=0.1), name="o1_w1o", dtype=tf.float32);
    ...
    def mlp_edge_B1(self,B):
     with tf.variable_scope("phi_E_R1") as scope:
       ...
       # tf.transpose(self.Et,[0,2,1]): (50,39800,200)
       h2_trans_bar1 = tf.matmul(h2_trans,tf.transpose(self.Es,[0,2,1])); # (50,20,200) B根据源端点排序 (50,20,39800)*(50,39800,200)
       h2_trans_bar2 = tf.matmul(h2_trans,tf.transpose(self.Et,[0,2,1])); # (50,20,200) B根据目的端点排序 (50,20,39800)*(50,39800,200)
    ...
    def mlp_hunk_B2(self,B2):
     with tf.variable_scope("mlp_hunk_B2") as scope:
       ...
       h2_trans_bar1 = tf.matmul(h2_trans,tf.transpose(self.Cs,[0,2,1])); # (50,20,74) B2根据源端hunk排序 (50,20,5402)*(50,5402,74)
       h2_trans_bar2 = tf.matmul(h2_trans,tf.transpose(self.Ct,[0,2,1])); # (50,20,74) B2根据目的端hunk排序 (50,20,5402)*(50,5402,74)
    ...
    def agg_edge_B1(self,E,Ra):
      C_R = tf.concat([Ra,E],1)
      return (C_R);

    def mlp2_edge_B1(self,C_R):
     with tf.variable_scope("phi_U_R1") as scope:
       ...
       # 要定义成变量，它才是一个变量
       w1 = tf.Variable(tf.truncated_normal([(self.De_er+self.Dr), h_size], stddev=0.1), name="o1_w1r", dtype=tf.float32);
    ...
    def mlp_hunkedge_B2(self, hunkedge):
        with tf.variable_scope("phi_U_R1") as scope:
            ...
            # 要定义成变量，它才是一个变量
            w1 = tf.Variable(tf.truncated_normal([hunkedge.shape[1], h_size], stddev=0.1), name="C_edge_w1", dtype=tf.float32); # (22,20)
            b1 = tf.Variable(tf.zeros([h_size]), name="C_edge_b1", dtype=tf.float32);  # (20)
            h1 = tf.nn.relu(tf.matmul(C_edge_trans2, w1) + b1);  # (273800,22)*(22,20) + 20 = (273800,20) 每个batch中所有hunk边的维度
    ...
    def train(self, args):# 激活(sess.run) model里面的参数
        ...
        for i in range(max_epoches):
          ...
          for j in range(int(len(E_node_train)/self.mini_batch_num)): # 按照batch划分轮数
            batch_O = E_node_train[j*self.mini_batch_num:(j+1)*self.mini_batch_num]; #(50,1,200) 取一个batch
          ...
          # with open(r'outputSelf/'+args.Repo+'/model_1/' + str(self.Step) + '/result_' + str(self.Step) + '.npy', "a", encoding='utf-8') as f:
          #     f.write(resultString)  # 存储实验结果

          # 存储实验结果
          # 文件路径
          filepath = r'outputSelf/{}/model_1/{}/result_{}.npy'.format(args.Repo, self.Step, self.Step)
    ...
    def save(self, checkpoint_dir, step):
        ...
        model_dir = "%s" % (self.Repo + '/model_1/'+ str(self.Step) )
    ...
    def test(self,args):
        ...
        for j in range(int(len(E_node_test)/self.mini_batch_num)):
          ...
          batch_C_edge_train = C_edge_test[j*self.mini_batch_num:(j+1)*self.mini_batch_num] #sess就像一个指针，处理的地方被激活
        ...
        # 定义目录和文件名
        base_dir = 'outputSelf/' + args.Repo + '/model_1/'
        ...
        # np.save('outputSelf/'+args.Repo+'/model_1/' + str(self.Step) + '/C_edge_t' + str(self.Ne) + '.npy', np.array(C_edge_t1).reshape(len(E_node_test), self.Dr, self.Ncr))
        # np.save('outputSelf/'+args.Repo+'/model_1/'+ str(self.Step) + '/C_edge_y' + str(self.Ne) + '.npy', C_edge_test.reshape(len(E_node_test), self.Dr, self.Ncr))
