...
def read_data(self, step):
    ...
    Ra_data = np.zeros((100, self.Dr, self.Nr), dtype=float32);  # (100,2,39800) batch_num, The Relationship Dimension, The Number of Relations
    ...
    Ra_label = np.zeros((100, self.Dr, self.HNr), dtype=float32);  # (100,2,5476) batch_num, The Relationship Dimension, The Number of Relations
    ...
    for i in bar6:
        ...
        for j in range(self.HNo):  # j 从 i+1 开始计算
            if (i != j):  # 每个图k的源端点和目的端点 Rr_data 和 Rs_data 都一样
                ...
                Hr_label[:, i, cnt1] = 1.0;  # 74是源端点i (100,74,5476)  batch_num, The Number of Objects, The Number of Relations
                Hs_label[:, j, cnt1] = 1.0;  # 74是目的端点j (100,74,5476)  batch_num, The Number of Objects, The Number of Relations
                for k in range(y_label.shape[0]):  # 遍历batch
                    # int(y_label[k,i,j]) 是 0 或 1。3个维度是确定的，得到 Relationship Dimension
                    # Ra_data (取自x) != Ra_label (取自y)。
                    Ra_label[k, int(y_label[k, i, j]), cnt1] = 1  # (100,2,5476) batch_num, The Relationship Dimension, The Number of Relations
                cnt1 += 1
