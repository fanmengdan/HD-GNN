# -*- coding: utf-8 -*-
# 有 entity embedding layer、 无 syntatic relation embedding layer(HD-GNN/S)
...
class graph2graph(object):
    ...
    def build_model(self):  # output 是 E_node_train2 和 E_edge_train2
        ...
        # entity 边源端、目的端的hunk
        self.Esc = tf.placeholder(tf.float32, [self.mini_batch_num, self.Nc, self.Ner],
                                  name="Esc_data")  # Esc_data, (50,74,39800)
        self.Etc = tf.placeholder(tf.float32, [self.mini_batch_num, self.Nc, self.Ner],
                                  name="Etc_data")  # Etc_data, (50,74,39800)
        ...
        # updating the entity state (node translation) E_node_train -> E_node_train2
        ...
        self.B_2 = self.marshalling_B1(self.E_node_train2, self.Es, self.Et, self.E_edge_train)  # 和 self.B_1 计算过程一样
    ...
    # 给mlp的输出的hunk边的表示 降维
    def mlp_hunkedge_B2(self, HRa):
        with tf.variable_scope("phi_U_R1") as scope:
            h_size = 20
            C_edge_trans = tf.transpose(HRa, [0, 2, 1])  # (50,22,5402)->(50,5402,22)
            C_edge_trans2 = tf.reshape(C_edge_trans, [self.mini_batch_num * self.Ncr, HRa.shape[1]])  # (50*5402,22)

            # 要定义成变量，它才是一个变量
            w1 = tf.Variable(tf.truncated_normal([HRa.shape[1], h_size], stddev=0.1), name="C_edge_w1",
    ...
    def train(self, args):  # 激活(sess.run) model里面的参数
        ...
        Es_data, Et_data, \
        Cs_label, Ct_label, \
        Esc_data, Etc_data = read_data(self, self.Step) # input 入口
        ...
        for i in range(max_epoches):
            ...
            acc_top = top_ACC(C_edge_train, np.array(C_edge_t).reshape(C_edge_train.shape[0], C_edge_train.shape[1],
                                                                      C_edge_train.shape[2]))  # (50,2,5402)
            ...
            # with open(r'outputSelf/'+args.Repo+'/model_2/'+ str(self.Step) +'/result_' + str(self.Step) + '.npy', "a", encoding='utf-8') as f:
            #     f.write(resultString) # 存储实验结果

            # 存储实验结果
            # 文件路径
            filepath = r'outputSelf/{}/model_2/{}/result_{}.npy'.format(args.Repo, self.Step, self.Step)
        ...
        end_time1 = time.time()
        print('test time:' + str(end_time1 - start_time1))


    def save(self, checkpoint_dir, step):
        model_name = "g2g.model"
        model_dir = "%s" % (self.Repo + '/model_2/'+ str(self.Step) )
    ...
    def test(self, args):
        ...
        # 定义目录和文件名
        base_dir = 'outputSelf/' + args.Repo + '/model_2/'
        ...
        # np.save('outputSelf/'+args.Repo+'/model_2/' + str(self.Step) + '/C_edge_t' + str(self.Ne) + '.npy', np.array(C_edge_t1).reshape(len(E_node_test), self.Dr, self.Ncr))
        # np.save('outputSelf/'+args.Repo+'/model_2/' + str(self.Step) + '/C_edge_y' + str(self.Ne) + '.npy', C_edge_test.reshape(len(E_node_test), self.Dr, self.Ncr))

        # evaluate, 从这里可以看出，最终生成的只有 O_t 和 Ra_t, t 是 target 的意思
        C_edge_t2 = process_edge(C_edge_t1)  # ?

        # print('mse-edge: ' + str(mse(C_edge_test, C_edge_t)))
        # print('r2-edge: ' + str(r2(C_edge_test, C_edge_t)))
        # print('p-edge: ' + str(pear(C_edge_test, C_edge_t)))
        # print('sp-edge: ' + str(spear(C_edge_test, C_edge_t)))
        ...
        # print('mse-node2: '+str(mse(node_label_test,E_node_test)))
        # print('topol_acc2: '+str(top_ACC(E_edge_test,C_edge_test)))
