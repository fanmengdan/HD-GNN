# -*- coding: utf-8 -*-
# 无 entity embedding layer、有 syntatic relation embedding layer (HD-GNN/E)
...
class graph2graph(object):
    ...
    def build_model(self):  # output 是 E_node_train2 和 E_edge_train2
        ...
        # self.a_E_node_train = self.agg_entity_B1(self.e_E_node_train
        # , self.Es, self.E_node_train)  # aggregation函数 (50,21,200)
        ...
        self.E_edge_train2, _ = self.mlp2_edge_B1(self.a_E_edge) # 降维, score被softmax后(50,2,39800), 输出的score(50,2,39800)

        # marshalling function
        # self.B_2 = self.marshalling_B1(self.E_node_train2, self.Es, self.Et, self.E_edge_train2)  # 和 self.B_1 计算过程一样
        self.B_3 = self.marshalling_B2(self.B_1, self.Esc, self.Etc, self.Cs, self.Ct, self.C_edge_train)  # !!!
    ...
    def train(self, args):  # 激活(sess.run) model里面的参数
        ...
        for i in range(max_epoches):
            ...
            for j in range(int(len(E_node_train) / self.mini_batch_num)):  # 按照batch划分轮数
                ...
                batch_C_edge_train = C_edge_train[j * self.mini_batch_num:(
                                                                                     j + 1) * self.mini_batch_num]  # (50,2,39800)sess就像一个指针，处理的地方被激活
            ...
            acc_top = top_ACC(C_edge_train,
                              np.array(C_edge_t).reshape(C_edge_train.shape[0], C_edge_train.shape[1],
            ...
            # with open(r'outputSelf/'+args.Repo+'/model_3/'+ str(self.Step) +'/result_' + str(self.Step) + '.npy', "a", encoding='utf-8') as f:
            #     f.write(resultString)  # 存储实验结果

            # 存储实验结果
            # 文件路径
            filepath = r'outputSelf/{}/model_3/{}/result_{}.npy'.format(args.Repo, self.Step, self.Step)
    ...
    def save(self, checkpoint_dir, step):
        ...
        model_dir = "%s" % (self.Repo + '/model_3/'+ str(self.Step) )
    ...
    def test(self, args):
        ...
        # 定义目录和文件名
        base_dir = 'outputSelf/' + args.Repo + '/model_3/'
        ...
        # np.save('outputSelf/'+args.Repo+'/model_3/'+ str(self.Step) +'/C_edge_t' + str(self.Ne) + '.npy', np.array(C_edge_t1).reshape(len(E_node_test), self.Dr, self.Ncr))
        # np.save('outputSelf/'+args.Repo+'/model_3/'+ str(self.Step) +'/C_edge_y' + str(self.Ne) + '.npy', C_edge_test.reshape(len(E_node_test), self.Dr, self.Ncr))
